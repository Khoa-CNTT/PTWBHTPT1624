...
const BannerModal: React.FC<BannerModalProps> = ({ isOpen, closeModal, onSave, banner }) => {
  ...
  const handleSave = () => {
    const { banner_description, banner_endDate, banner_imageUrl, banner_link, banner_startDate, banner_title } =
      inputFields;
  ...
  const isDateValid =
    inputFields.banner_startDate &&
    inputFields.banner_endDate &&
    new Date(inputFields.banner_startDate) <= new Date(inputFields.banner_endDate);
