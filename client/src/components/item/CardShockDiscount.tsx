...
const CardShockDiscount: React.FC<{ product: IProductItem }> = ({ product }) => {
    const { product_discount, _id, product_price, product_thumb, product_slug, product_quantity, product_sold } = product;
    ...
    return (
        <Link to={`${product_slug}/${_id}`} className="flex flex-col w-full h-full gap-2 px-3 rounded-sm hover:shadow-cart cursor-pointer">
            ...
            <div className="relative mb-2">
                <button className="relative overflow-hidden flex w-full text-sm py-1 text-white  bg-[rgb(255,122,131)]   rounded-2xl justify-center items-center hover:opacity-80 z-10">
