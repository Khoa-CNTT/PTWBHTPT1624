...
import { NavItem, navItems } from '../../config/navItems';
...
const AppSidebar: React.FC = () => {
    ...
    const checkPermission = (permission: any) => {
        if (admin?.admin_type === 'admin' || admin?.permissions?.includes(permission)) return true;
        return false;
    };
    ...
    const renderMenuItems = (items: NavItem[], menuType: 'main' | 'others') => (
        <ul className="flex flex-col gap-2">
            {items.map((nav, index) => {
                if (!checkPermission(nav.permission)) return <></>;
                return (
                    <li key={nav.name}>
                        {nav.subItems ? (
                            <button
            ...
            <div className={`py-4 flex mx-auto ${!isExpanded && !isHovered ? 'lg:justify-center' : 'justify-start'}`}>
                <Link to="/quan-ly">
                    {isExpanded || isHovered || isMobileOpen ? (
                        ...
                        <img src={iconLogo} alt="Logo" width={32} height={32} />
                    )}
                </Link>
            </div>
